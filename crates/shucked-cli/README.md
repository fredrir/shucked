...
`shucked` is the command-line shell checker and formatter in the Shucked workspace.

This crate is published so the binary, integration tests, and benchmarks can share the same
argument parsing and command execution logic. Most users should install and run the `shucked`
