    ...
    match command {
        ...
        TargetCommand::Compare { targets, script } => {
            ...
            let facts = if let Some(fish) = fish {
                ...
                let parsed = shucked_parser::parser::Parser::with_profile(&source, profile.clone())
            ...
            let sites: Vec<_> = facts
                .iter()
                .map(|fact| CommandSite {
                    name: fact.name().map(str::to_owned),
                    arguments: fact
                        .effective_words
                        .iter()
                        .skip(1)
                        .filter_map(|word| word.text.clone())
                        .collect(),
                    ...
                    guarded: if fact.guarded_available {
                        fact.name().into_iter().map(str::to_owned).collect()
                    } else {
                        Default::default()
                    },
                    lookup: match fact.namespace {
                        shucked_semantic::CommandNamespace::Builtin => {
                            shucked_command::LookupMode::BuiltinOnly
                        }
                        shucked_semantic::CommandNamespace::External => {
                            shucked_command::LookupMode::ExternalOnly
                        }
                        shucked_semantic::CommandNamespace::ExternalOrBuiltin => {
                            shucked_command::LookupMode::Command
                        }
                        _ => shucked_command::LookupMode::Normal,
                    },
                    ..Default::default()
                })
                .collect();
...
fn read_inventory(path: &Path) -> Result<TargetInventory> {
    ...
    std::fs::File::open(path)?
        .take((shucked_command::MAX_INVENTORY_BYTES + 1) as u64)
