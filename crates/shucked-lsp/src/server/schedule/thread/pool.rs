...
impl Pool {
    pub(crate) fn new(threads: NonZeroUsize) -> Self {
        ...
        for index in 0..threads {
            handles.push(spawn_worker(index, job_receiver.clone()));
        }
    ...
    pub(crate) fn spawn<F>(&self, _priority: ThreadPriority, f: F)
    where
        F: FnOnce() + Send + 'static,
    {
        if let Err(error) = self.job_sender.send(Job { f: Box::new(f) }) {
