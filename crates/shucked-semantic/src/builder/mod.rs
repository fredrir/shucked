...
use shucked_parser::{
    ShellDialect, ShellProfile, ZshEmulationMode, ZshOptionState, parser::Parser,
};
...
pub(crate) struct BuildOutput {
    ...
    pub(crate) source_path_expressions: FxHashMap<BindingId, SourcePathTemplate>,
...
impl<'a, 'idx, 'observer> SemanticModelBuilder<'a, 'idx, 'observer> {
    pub(crate) fn build(
        ...
        let mut builder = Self {
            ...
            source_path_expressions: FxHashMap::default(),
        ...
        BuildOutput {
            ...
            source_path_expressions: builder.source_path_expressions,
...
fn binding_attributes_for_var_ref(reference: &VarRef) -> BindingAttributes {
    ...
    {
        Some(shucked_ast::SubscriptInterpretation::Associative) => {
...
fn simple_command_has_name(command: &shucked_ast::SimpleCommand, source: &str) -> bool {
...
fn single_literal_word(word: &Word) -> Option<&str> {
    match word.parts.as_slice() {
        [part] => match &part.kind {
            WordPart::Literal(
                shucked_ast::LiteralText::Owned(text)
                | shucked_ast::LiteralText::CookedSource(text),
            ) => Some(text.as_ref()),
            _ => None,
        },
        _ => None,
