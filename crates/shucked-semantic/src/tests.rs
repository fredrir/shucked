...
#[test]
fn shellcheck_source_directive_keeps_shellcheck_origin() {
    ...
    assert_eq!(
        ...
        "shellcheck source= is not the shuck-native spelling"
    );
    assert!(!refs[0].has_shuck_directive());
}
...
";
    ...
    let break_span = match &for_command.body[0].command {
        Command::Builtin(shucked_ast::BuiltinCommand::Break(command)) => command.span,
...
fn plugin_source_resolution_does_not_apply_to_non_zsh_sources() {
    ...
    let model = SemanticModel::build_with_options(
        ...
        SemanticBuildOptions {
            ...
            shell_profile: Some(ShellProfile::native(shucked_parser::ShellDialect::Bash)),
            ..SemanticBuildOptions::default()
        },
    );
