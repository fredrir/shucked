...
use shucked_ast::{Command, ConditionalExpr, File, Name, Span, Stmt, TextRange};
...
pub struct SemanticModel {
    ...
    command_alias_offsets: OnceLock<std::collections::BTreeMap<String, usize>>,
...
impl SemanticModel {
    ...
    fn from_build_output(built: builder::BuildOutput) -> Self {
        ...
        Self {
            ...
            command_alias_offsets: OnceLock::new(),
...
fn infer_parse_dialect_from_source(
    source: &str,
    path: Option<&Path>,
) -> shucked_parser::ShellDialect {
    if let Some(interpreter) = shebang_interpreter(source) {
        return parse_dialect_from_name(interpreter).unwrap_or(shucked_parser::ShellDialect::Bash);
    }

    infer_parse_dialect_from_path(path).unwrap_or(shucked_parser::ShellDialect::Bash)
}

pub(crate) fn infer_explicit_parse_dialect_from_source(
    source: &str,
    path: Option<&Path>,
) -> Option<shucked_parser::ShellDialect> {
    if let Some(interpreter) = shebang_interpreter(source)
...
fn shebang_interpreter(source: &str) -> Option<&str> {
    ...
    shucked_parser::shebang::interpreter_name(first_line.strip_suffix('\r').unwrap_or(first_line))
}

fn infer_parse_dialect_from_path(path: Option<&Path>) -> Option<shucked_parser::ShellDialect> {
    match path
        .and_then(|path| path.extension().and_then(|ext| ext.to_str()))
        .map(|ext| ext.to_ascii_lowercase())
        .as_deref()
    {
        Some("sh" | "dash" | "ksh") => Some(shucked_parser::ShellDialect::Posix),
        Some("mksh") => Some(shucked_parser::ShellDialect::Mksh),
        Some("bash") => Some(shucked_parser::ShellDialect::Bash),
        Some("zsh") => Some(shucked_parser::ShellDialect::Zsh),
        _ => None,
    }
}

fn parse_dialect_from_name(name: &str) -> Option<shucked_parser::ShellDialect> {
    match name.to_ascii_lowercase().as_str() {
        "sh" | "dash" | "ksh" | "posix" => Some(shucked_parser::ShellDialect::Posix),
        "mksh" => Some(shucked_parser::ShellDialect::Mksh),
        "bash" => Some(shucked_parser::ShellDialect::Bash),
        "zsh" => Some(shucked_parser::ShellDialect::Zsh),
        _ => None,
    }
}
