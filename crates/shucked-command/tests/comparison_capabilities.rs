...
fn comparison_distinguishes_recorded_flag_incompatibility_from_unknown_coverage() {
    ...
    snapshot.exact_lookups.insert(
        ...
        LookupEvidence::Present(Executable {
            ...
            provenance: Provenance::new("captured"),
        }),
    );
    ...
    snapshot.validators.insert(
        ...
        ValidationEvidence {
            ...
            grammar: CommandGrammar {
                flags: BTreeMap::from([("--old".into(), FlagSpec::default())]),
                flags_complete: true,
    ...
    assert!(
        comparison.commands[0]
            .results
            .iter()
            .all(|result| matches!(result, CommandResolution::Resolved(_)))
    );
}
