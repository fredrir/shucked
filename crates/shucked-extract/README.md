...
`shucked-extract` extracts shell scripts embedded in non-shell host files.

Today it provides the extraction layer behind embedded GitHub Actions support in `shucked check`.
...
an opaque shell-analysis projection, parses each expression into an owned `shucked-ast` tree, and
...
The crate is part of the published Shucked toolchain, but its Rust API is still pre-1.0 and may
