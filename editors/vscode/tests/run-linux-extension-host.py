...
'''
    ...
    wrapper.chmod(0o700)
    receipt = {'target': target, 'buildMode': arguments.build_mode, 'serverSha256': digest(arguments.server), 'workerServerSha256': digest(arguments.worker_server), 'bundledServerSha256': digest(binaries / 'shucked'), 'bundledWorkerServerSha256': digest(binaries / 'shucked-server'), 'vsixSha256': digest(vsix), 'vsix': str(vsix), 'sourceRevision': subprocess.check_output(['git', '-C', repository, 'rev-parse', 'HEAD'], text=True).strip(), 'node': subprocess.check_output([arguments.node, '--version'], text=True).strip(), 'passed': False}
